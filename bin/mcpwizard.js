...
import { generateClaudeDesktopConfigCommand, getDefaultConfigPath } from '../src/commands/generate-claude-config-file.js';
...
program
  ...
  .option('-t, --template <template>', 'Specify a template (typescript, python)', 'typescript')
  .action((name, options) => {
...
program
  ...
  .action((action, name, options) => {
    if (action === 'add' && name) {
      ...
      console.error(chalk.red(`Unknown action or missing name: ${action} ${name || ''}`));
      process.exit(1);
    }
  });
